test_that("mermaid_search_projects throws a message if it returns more than one project, and doesn't if it only returns one", {
...
test_that("mermaid_search_projects returns a zero row tibble if the project is not found", {
  ...
  output <- mermaid_search_projects(id = id)
  expect_equal(nrow(output), 0)
  expect_is(output, "tbl_df")
})
