...
test_that("mermaid_get_endpoint returns the correct columns given the endpoint.", {
  ...
  output <- mermaid_get_endpoint("choices")
  expect_equal(names(output), mermaid_endpoint_columns[["choices"]])
  ...
  output <- mermaid_get_endpoint("fishsizes", limit = 1)
  expect_equal(names(output), mermaid_endpoint_columns[["fishsizes"]])
  ...
  output <- mermaid_get_endpoint("projecttags", limit = 1)
  expect_equal(names(output), mermaid_endpoint_columns[["projecttags"]])
