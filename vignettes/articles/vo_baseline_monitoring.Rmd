...
We are able to see the data policy of projects and methods by looking at the `data_policy_*` columns of `vo_projects`. For example, focusing on benthic PIT and fishbelt, we can see that the `r both_public_summary` project has summary data publicly available for fishbelt and benthic PIT, while the `r only_pit_public_summary` project has public data available for only benthic PIT.
...
We'll focus on hard coral (benthic PIT) data first. We can get this data by filtering for projects that have publicly available summary data for it:
...
benthic_data_latest <- benthic_data_latest %>%
  mutate(
    ...
    threshold = fct_expand(threshold, "Below 10% cover", "Midrange 10-30% cover", "Above 30% hard coral cover")
  )
```

```{r}
benthic_data_latest %>%
  count(threshold, .drop = FALSE) %>%
  mutate(prop = n / sum(n))
```
...
benthic_data_latest <- benthic_data_latest %>%
  mutate(
    threshold = fct_relevel(threshold, c("Above 30% hard coral cover", "Midrange 10-30% cover", "Below 10% cover")),
    site = fct_reorder(site, hard_coral)
  )

hard_coral_plot <- ggplot(benthic_data_latest) +
  geom_col(aes(x = hard_coral, y = site, fill = threshold), alpha = 0.8, show.legend = TRUE) +
...
hard_coral_plot <- hard_coral_plot +
  ...
  scale_fill_manual(name = "Threshold", values = c("#7F7F7F", "#F1A83B", "#E73F25"), drop = FALSE) +
...
It might also be interesting to see the changes in hard coral cover over time. To make this graph, first, we need to go back to the original `benthic_data` data set which contains sample date, and extract the year.

```{r}
benthic_data_with_year <- benthic_data %>%
...
```{r message = FALSE}
hard_coral_by_country_by_year <- benthic_data_with_year %>%
...
In the `hard_coral_by_country_by_year` data frame, you can see that some countries only have one year of data. We will remove those countries.
...
```{r fig.alt="A line chart with year on the x axis and average hard coral cover on the y axis. The lines are colored in red and blue to differentiate per country. The plot has a title: Trend Average Hard Coral Cover per Country. The x axis is titled: Year and the y axis is titled: Average Hard Coral Cover (%)."}
ggplot(
  hard_coral_by_country_by_year,
...
```{r fig.alt="A bar chart with country on the x axis and average fish biomass on the y axis. The bars are colored in red, green, and blue to differentiate per country. The plot has a title: Average Fish Biomass per Country. The x axis is titled: Country and the y axis is titled: Average Fish Biomass (kg/ha)."}
...
We have our average fish biomass per country plot. Let's continue with visualizing the changes in fish biomass per country. Again, we'll go back to the original `fishbelt_data` data set which has the sample date, and extract the year.

```{r}
fishbelt_data_with_year <- fishbelt_data %>%
  select(country, site, sample_date, biomass = biomass_kgha_avg) %>%
  mutate(year = year(sample_date))

fishbelt_biomass_by_country_by_year <- fishbelt_data_with_year %>%
  group_by(country, year) %>%
  summarise(
    fish_biomass_average = mean(biomass),
    .groups = "drop"
...
Again, we will only use data from countries with more than one year of data, and visualize it in a line plot.

```{r fig.alt="A line chart with year on the x axis and average fish biomass on the y axis. The lines are colored in red and tosca to differentiate per country. The plot has a title: Trend of  Average Fish Biomass by Country. The x axis is titled: Year and the y axis is titled: Average Fish Biomass (kg/ha)."}
