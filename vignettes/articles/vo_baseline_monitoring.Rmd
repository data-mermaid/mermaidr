...
```{r}
hard_coral_by_country <- benthic_data %>%
  group_by(country) %>%
  summarise(
...
It might also be interesting to see the changes in hard coral cover over time. To make this graph, first, we need to add the data column in our benthic data.
...
After adding the date column, the next step is to separate it into year, month, and day. To do this, we're going to use the `separate()` function.
