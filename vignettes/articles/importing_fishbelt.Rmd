---
title: Importing legacy data into MERMAID
output: html_document
---



Do you have a lot of existing data and are just starting to use MERMAID? You might be wondering on how to import those data into MERMAID. You can import your legacy data into MERMAID by using the mermaidr package. More info on the importing workflow with mermaidr can be seen on the [importing legacy data into MERMAID documentation](https://datamermaid.org/documentation/mermaidr-data-import).

Here we provide an example on how to import your fish data that uses the **fish belt method** into MERMAID using mermaidr. Please note that you need to **prepare your project in the MERMAID Collect first** before starting this process. Detail on how to set a new project can be seen in the [Setting up a project documentation](https://datamermaid.org/documentation/collect-project-set-up). For this example, we have already prepared a project in the MERMAID Collect named **MERMAID reef survey**.

We have also prepared <a href="https://www.youtube.com/watch?v=bgmz0vhg2QI" target="_blank">a video</a> to walk you through the importing process. You can download the [fishbelt](https://github.com/data-mermaid/mermaidr/tree/master/examples/import_historical_fishbelt/fishbelt.csv) and [sites](https://github.com/data-mermaid/mermaidr/tree/master/examples/import_historical_fishbelt/sites.csv) data for that example to follow along.

## The steps for importing legacy fish belt data are:

* 1. Prepare your project in MERMAID Collect
* 2. Prepare your fish belt data in a CSV format
* 3. Install and activate mermaidr and other related packages
* 4. Download the fish belt MERMAID template
* 5. Prepare and reformat your data to match the MERMAID template
...
Before importing your fish belt data to MERMAID, you need to set up your project in [MERMAID Collect](https://app.datamermaid.org/). If you are new to MERMAID, you need to create a MERMAID account before using it. You can use a new project or existing project in MERMAID Collect as long as you are an admin of the project. [Setting up a new project](https://datamermaid.org/documentation/collect-project-set-up) requires internet access. To create a new project, click on the New Project button and then:
...
## Step 2. Prepare your fish belt data in a CSV format

If your data isn't in CSV format, you'll need to re-save it as CSV. This example uses a CSV file. However, if you prefer to use a different format, ensure that it can be read in R.


## Step 3. Install and activate mermaidr and other related packages

After setting up your project in MERMAID Collect and preparing the necessary files, you can now start importing your data in R or R Studio. The first step is to install and activate the mermaidr and other related packages. In this example, we are also going to use the tidyverse package.
...
## Step 4. Download the fish belt MERMAID template

Next step is to download the MERMAID Fish belt template. Make sure to set up your working directory to point R to where your data is located and where you want to save your data from R, including the MERMAID Fish belt template.

To download the MERMAID Fish belt template:
...
At this point, a browser window will open for you to authenticate by logging into the MERMAID Collect. Once you've logged in, you can close the browser and come back to R. Your login credentials will last for a day and after it expires you will need to log in again.
...
```
Before running the code above, make sure that you change the project name **“MERMAID reef survey”** to **_your actual project_** that you want to import your data into.
...
The XLSX file consists of the fish belt MERMAID template and the options that are tailored based on your project. For example, the site options that are accepted are all the sites that you have added in the project through the MERMAID Collect.
...
## 
## $help_text
## [1] "A unique name of a site where data was collected. Every site must be defined before importing and set up in the project in the MERMAID Collect."
## 
## $choices
## # A tibble: 6 × 1
##   value    
...
The MERMAID importing template requires the sample event data and the observation data to be combined. Therefore, we need to add the `sites` data set to fish observation data. Look at the available columns in the `site` and `fishbelt` data sets:
...
## 
## $help_text
## [1] "Width of fish belt transect, in meters. See relevant tab on importing template for choices."
## 
## $choices
## # A tibble: 9 × 1
##   value                                        
...
## 
## $help_text
## [1] "Name of bin scheme used to estimate fish size for the transect. See relevant tab on importing template for choices. Choose 1 cm if the fish size recorded does not use bins."
## 
## $choices
## # A tibble: 5 × 1
##   value    
...
Don't worry if you don't have any of `Sample time`, `Transect label`, `Relative depth`, `Tide`, `Sample unit notes` information, because none of them are required. It will not prevent you from importing your data into MERMAID. However, if you have those data, it is strongly recommended to add them as well.
...
There are issues in the Fish name that need to be fixed, marked by the `FALSE` note under the match column. MERMAID also provides the closest choice to help us with data cleaning. We need to fix theses issues to be able to import our data:
...
Once we got the check marks, we have the cleaned version of our data that is ready to be imported. Let's save the cleaned fishbelt data:
...
Once we have our cleaned MERMAID formatted data, the next step is importing the data. We do one "dry run" before actually importing to check the data once more:
...
Once we got the message **Records successfully checked!**, change the `dryrun` option to `FALSE` to start importing your data into MERMAID.
...
## Step 7. Validate and submit in MERMAID Collect
...
**Congratulations! You have successfully imported your fish belt data to MERMAID!**
